...
.. note::
    ...
    For steps with several branches (such as :class:`BranchingStep`, :class:`ChoiceSelectionStep`,
    :class:`RetryStep`, and :class:`FlowExecutionStep`), you need to mapping each branch name to
...
Most of the time, you will use :class:`BranchingStep` to branch out depending on whether a token is present
in a text (for example, whether an LLM generated a token ``[SUCCESS]`` or not).
To do this, pass :class:`RegexExtractionStep` before the :class:`BranchingStep`:
...
Mapping works the same as for :class:`BranchingStep`:
...
- :class:`BranchingStep`.
- :class:`BranchingStep` with pattern matching.
- more complex conditionals with :class:`ToolExecutionStep`, or :class:`TemplateRenderingStep`, and
  :class:`BranchingStep`.
- an LLM to decide on the condition using :class:`ChoiceSelectionStep`.
- a sub-flow to handle the conditional logic using :class:`FlowExecutionStep`.
