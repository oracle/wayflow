...
WayFlow allows LLMs to interact with applications via **Tools** ([ClientTool](api/tools.md#clienttool), [ServerTool](api/tools.md#servertool), or [RemoteTool](api/tools.md#remotetool)), granting access to state or operations. Since tool inputs often come from LLMs or users, rigorous input sanitization and validation are crucial.

**Key Principles for Tool Security:**

* **Mandatory Input Validation**: Always validate tool inputs.
  * For [ClientTool](api/tools.md#clienttool) and [ServerTool](api/tools.md#servertool), use `input_descriptors` to define/enforce schemas (types and descriptions) as a primary defense. Note that `input_descriptors` do not support constraints like ranges or max lengths, so you must implement additional validation in your tool’s code.
  * For [RemoteTool](api/tools.md#remotetool), which constructs HTTP requests, validation is critical for all parameters that can be templated (e.g., `url`, `method`, `json_body`, `data`, `params`, `headers`, `cookies`). While `input_descriptors` can define the schema for arguments *passed to* the [RemoteTool](api/tools.md#remotetool), the core security lies in controlling how these arguments are used to form the request and in features like `url_allow_list`.
* **Output Scrutiny**: Define expected outputs with `output_descriptors`. For [ClientTool](api/tools.md#clienttool), clients post results; for [ServerTool](api/tools.md#servertool), the server `func` generates them. Calling Flows/Agents must treat incoming [ToolResult](api/tools.md#toolresult) content as untrusted until validated/sanitized.
...
* **Callable Security (\`func\`)**: The `func` callable in [ServerTool](api/tools.md#servertool) is the primary security concern. Harden this server-executed code against vulnerabilities.
* **Isolation for High-Risk Tools**: Run high-risk [ServerTool](api/tools.md#servertool) instances (e.g., with elevated permissions, network/filesystem access) in sandboxed environments (containers/pods) with minimal IAM roles. Deny network/filesystem writes unless essential.
* **Tools from \`Flows\` or \`Steps\`**:
  * When creating a [ServerTool](api/tools.md#servertool) via [`from_flow()`](api/tools.md#wayflowcore.tools.servertools.ServerTool.from_flow), or [`from_step()`](api/tools.md#wayflowcore.tools.servertools.ServerTool.from_step), its security is inherited. Ensure source `Flow` or `Step` tools are secure.

**\`ClientTool\` Considerations:**

* **Client-Side Execution**: [ClientTool](api/tools.md#clienttool) execution occurs on the client. Client environment security, though outside WayFlow’s control, impacts overall application security.
* **Untrusted \`ToolRequest\`**: Clients receive a [ToolRequest](api/tools.md#toolrequest). Though WayFlow generates `name` and `tool_request_id`, client code must parse `args` with a strict schema, avoiding direct use in shell commands or sensitive OS functions.
* **Untrusted Client \`ToolResult\`**: Server-side WayFlow components must treat [ToolResult](api/tools.md#toolresult) from clients as untrusted. Validate its `content` before processing.

**\`RemoteTool\` Considerations:**

* **Templated Request Arguments**: [RemoteTool](api/tools.md#remotetool) allows various parts of the HTTP request (URL, method, body, headers, etc.) to be templated using Jinja. This is powerful but introduces risks if the inputs to these templates are not strictly controlled. Maliciously crafted inputs could lead to information leakage (e.g., exposing sensitive data in URLs or headers) or enable attacks like SSRF (Server-Side Request Forgery) or automated DDoS.
...
* **Secure Connections (\`allow_insecure_http\`)**: By default, [RemoteTool](api/tools.md#remotetool) disallows non-HTTPS URLs (`allow_insecure_http=False`). Maintain this default unless there’s an explicit, well-justified reason to allow insecure HTTP, and ensure the risks are understood.
...
#### CAUTION
As highlighted in the [RemoteTool](api/tools.md#remotetool) API, since the Agent can generate arguments
...
vectors like automated DDOS attacks. Please use [RemoteTool](api/tools.md#remotetool) responsibly and ensure
...
* For [ClientTool](api/tools.md#clienttool) & [ServerTool](api/tools.md#servertool): Use `input_descriptors` for basic type checking. In [ServerTool](api/tools.md#servertool)’s `func`, add comprehensive validation (string length limits, numeric ranges, format constraints). Cap string lengths and numeric ranges in tool implementation code.
* For [RemoteTool](api/tools.md#remotetool):
...
Excessive privileges (for [ServerTool](api/tools.md#servertool))
...
[ClientTool](api/tools.md#clienttool) misuse (client-side vulnerabilities)

Client apps handling [ToolRequest](api/tools.md#toolrequest) must treat `args` as untrusted.
...
Ensure source Flows or Steps tools for WayFlow [ServerTool](api/tools.md#servertool) are vetted; the tool inherits their security.

Data leakage via [ToolResult](api/tools.md#toolresult)

Define `output_descriptors` clearly.

Ensure [ServerTool](api/tools.md#servertool) `func` and [ClientTool](api/tools.md#clienttool) client code return only necessary data.

Consuming Agent/Flow must validate/sanitize [ToolResult](api/tools.md#toolresult) content.

## Considerations regarding network communication

WayFlow components use the network for LLM communications, [ApiCallStep](api/flows.md#apicallstep) tool operations, and third-party integrations. Implement robust network security.
...
* **Subnet Isolation**:
  : * LLM hosts (OCI Gen AI, vLLM) in dedicated subnets.
    * Tool backends and [ApiCallStep](api/flows.md#apicallstep) targets in isolated subnets.
...
* **Strict Egress Rules**: Default-deny outbound traffic:
  : * Allow only Allow-Listed domains/IPs for WayFlow components (such as when using Tools like [RemoteTool](api/tools.md#remotetool) with its `url_allow_list` parameter, [ApiCallStep](api/flows.md#apicallstep), or [PromptExecutionStep](api/flows.md#promptexecutionstep))
* **Centralized Allow-Lists for WayFlow**:
  : * LLM provider endpoints and Telemetry/logging destinations.
    * Approved Tool API endpoints for tools making external calls, such as [ApiCallStep](api/flows.md#apicallstep) and [RemoteTool](api/tools.md#remotetool) (enforced via its `url_allow_list` parameter).
...
External API Call Security (e.g., [ApiCallStep](api/flows.md#apicallstep), [RemoteTool](api/tools.md#remotetool))

• Enforce HTTPS: Use `allow_insecure_http=False` (default for both).
• **URL Allow Lists**: For [RemoteTool](api/tools.md#remotetool), always configure its `url_allow_list` parameter. For [ApiCallStep](api/flows.md#apicallstep), ensure broader network-level allow-lists are in place.
• Validate/sanitize templated URLs and other request parameters (headers, body) derived from potentially untrusted inputs.
• Consider `allow_credentials=False` and `allow_fragments=False` for [RemoteTool](api/tools.md#remotetool) if those features are not strictly necessary.
...
* Monitor [TokenUsage](api/llmmodels.md#tokenusage) for anomaly detection.
* Secure proxy credentials separately for VPN proxies.

**OCI GenAI Models (\`OCIGenAIModel\`):**

* **Instance Principal** ([OCIClientConfigWithInstancePrincipal](api/llmmodels.md#ociclientconfigwithinstanceprincipal)): Preferred for OCI Compute (no stored credentials).
* **API Key** ([OCIClientConfigWithApiKey](api/llmmodels.md#ociclientconfigwithapikey)): Store private keys in key management; protect `~/.oci/config` (permissions 600).
* **User Authentication** ([OCIClientConfigWithUserAuthentication](api/llmmodels.md#ociclientconfigwithuserauthentication)): Never log `key_content`.
...
* **Monitoring**: Track WayFlow’s [TokenUsage](api/llmmodels.md#tokenusage) for anomalies.
...
* **Concurrency guard**
  ...
  For WayFlow’s [MapStep](api/flows.md#mapstep) or similar fan-out patterns:
  ```python
...
* **Input size validation**
  ...
  Limit list lengths, string sizes, and recursion depth for data fed into WayFlow’s [MapStep](api/flows.md#mapstep) or recursive Agents to prevent exponential resource use.
...
* Use `output_descriptors` on [PromptExecutionStep](api/flows.md#promptexecutionstep) for JSON schemas to reduce prompt-injection leakage.
...
* **LLM DevSecOps**: Apply [LLM DevSecOps guide](https://devsecopsguides.com/docs/rules/llm/) practices to WayFlow development.

**Architectural Patterns for Input Security:**

* **Early filtering**: Insert a [BranchingStep](api/flows.md#branchingstep) at Flow start to route “unsafe” content to a refuse/retry branch.
...
[SpanExporter](api/tracing.md#spanexporter).
...
When you add a custom [SpanProcessor](api/tracing.md#spanprocessor):
...
* [TemplateRenderingStep](api/flows.md#templaterenderingstep)
* [OutputMessageStep](api/flows.md#outputmessagestep)
* [ContextProvider](api/contextproviders.md#contextprovider)-driven templates interpolating values into messages
...
1. **Segregate rendering responsibilities**
...
2. **Sanitise any LLM or user-supplied markup**
...
3. **Adopt strict content policies**
...
4. **Use an “escape hatch” for deliberate rich content**
...
5. **Audit TemplateRenderingStep usage**
...
* **\`ServerTool\` results**
  ...
  [ServerTool](api/tools.md#servertool) instances may return Markdown or HTML. Mark the result type explicitly (e.g., `text/plain` vs `text/markdown`) and render with a Markdown library that sanitises HTML by default (e.g., `markdown-it-py` with `linkify=False`).

## Considerations regarding assistant/flow serialization

WayFlow components inheriting from [SerializableObject](api/serialization.md#serializableobject) can be converted to/from a dictionary representation (typically YAML) using [serialize](api/serialization.md#serialize).
...
4. **Controlled Deserialization Context**:
   While [autodeserialize](api/serialization.md#autodeserialize) accepts a `deserialization_context`, ensure a reused context doesn’t carry over state from a prior untrusted deserialization that could be exploited.
...
Remember that [autodeserialize](api/serialization.md#autodeserialize) expects parsed YAML (a Python dictionary), not the raw YAML string, if using `safe_load`. Passing a string directly to `autodeserialize` makes it use `yaml.safe_load` internally. Explicit `safe_load` first allows pre-validation of the YAML structure.

## Other Security Concerns

For any other security concerns, please submit a [GitHub issue](https://github.com/oracle/wayflow/issues).

| Class                                                                                  | Purpose / behaviour                                                                                                                                                                                                                       |
|----------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [SoftTimeoutExecutionInterrupt](api/interrupts.md#softtimeoutexecutioninterrupt)       | Stops the assistant **after** a configurable wall-clock duration (default 10 min).
...
| [SoftTokenLimitExecutionInterrupt](api/interrupts.md#softtokenlimitexecutioninterrupt) | Aborts execution once an LLM (or group of LLMs) has emitted a specified number of tokens.
