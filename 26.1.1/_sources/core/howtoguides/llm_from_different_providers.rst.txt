...
.. option:: proxy: str, optional

  Proxy settings to access the remote model.

.. option:: api_type: OpenAIAPIType

  OpenAI API type to use. Should be one of the values from :ref:`OpenAIAPIType<openaiapitype>`. Defaults to `Chat Completions API <https://platform.openai.com/docs/api-reference/chat>`_
...
As of November 2025, Ollama does not support `OpenAI Responses API <https://platform.openai.com/docs/api-reference/responses>`_. Therefore only Chat Completions API will be supported for OpenAI models.
