...
def _get_component_reference_ids(component: "Component") -> set[str]:
    deserialization_context = DeserializationContext()
    deserialization_context._add_component_to_context(component)
    return set(deserialization_context._deserialized_objects.keys())
...
def _build_checkpoint_serialization_context(conversation: "Conversation") -> SerializationContext:
    ...
    for component in _iter_component_tree(conversation.component):
        serialization_context.record_obj_dict(component, {})
    return serialization_context


def _strip_component_tree_from_serialized_conversation(
    serialized_conversation: Dict[str, Any],
    component: "Component",
) -> Dict[str, Any]:
    referenced_objects = dict(serialized_conversation.get("_referenced_objects", {}))
    if not referenced_objects:
        return serialized_conversation

    component_refs = _get_component_reference_ids(component)
    for component_ref in component_refs:
        referenced_objects.pop(component_ref, None)

    if referenced_objects:
        serialized_conversation["_referenced_objects"] = referenced_objects
    else:
        serialized_conversation.pop("_referenced_objects", None)
...
def _serialize_conversation_checkpoint_state(conversation: "Conversation") -> str:
    ...
    serialized_conversation = _strip_component_tree_from_serialized_conversation(
        serialized_conversation, conversation.component
    )
