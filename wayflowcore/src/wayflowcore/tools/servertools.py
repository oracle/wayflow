...
class ServerTool(Tool):
    ...
    async def run_async(self, *args: Any, **kwargs: Any) -> Any:
        ...
        """
        # Reject synchronous generators: streaming requires async generators
        if inspect.isgeneratorfunction(self.func):
            raise TypeError(
                "Synchronous generator tools are not supported for streaming; use 'async def' with 'yield'"
            )
    ...
    def run(self, *args: Any, **kwargs: Any) -> Any:
        ...
        # Async-generator tools require the async execution path to preserve
        # ToolExecutionSpan context and emit streaming events reliably.
        if inspect.isasyncgenfunction(self.func):
            raise TypeError(
                "Async-generator tools must be executed via 'run_async' to support streaming "
                "and span correlation; synchronous 'run' is not supported for streaming tools."
            )
    ...
    async def _run_async_generator_streaming(self, *args: Any, **kwargs: Any) -> Any:
        ...
        Behavior:
        - Pulls the first two yielded items up-front to distinguish single-yield generators
          (which produce only a final result) from multi-yield generators (which have
          intermediate chunks). If only one item is yielded, it is returned directly with
          no streaming events.
        - If two or more items are present, the method emits streaming events for every
          ...
          output. This preserves the contract that the final yielded value is the
          authoritative tool result while providing progressive updates.
        - Enforces a configurable chunk cap via a ContextVar; -1 disables the cap.

        Rationale for pulling the second chunk:
        - Emitting the first item immediately could be incorrect if the generator stops
          after that one item (that would be the final output, not a chunk). Pulling a
          second item ensures we only emit the first as a chunk when we know there is a
          subsequent item, leaving the last to be returned as the final result.
        """
        ...
        agen: AsyncGenerator[Any, None] = self.func(*args, **kwargs)

        # Return last yielded item as final output; stream prior items as events
        chunk_count = 0
        ...
        try:
            second_chunk = await agen.__anext__()
        except StopAsyncIteration:
            return self._add_defaults_to_tool_outputs(first_chunk)

        def _emit_chunk(content: Any) -> None:
        ...
        _emit_chunk(first_chunk)
        chunk_count += 1
        previous_chunk = second_chunk
        while True:
            if max_allowed_chunks != -1 and chunk_count >= max_allowed_chunks:
                await agen.aclose()  # close generator to avoid background work/leaks
                raise ValueError(
