...
def test_agent_state_snapshots_support_the_agui_retrieval_export_flow() -> None:
    ...
    agent_span = single_span(span_recorder, AgentSpecAgentExecutionSpan)
    assert events(agent_span, AgentSpecAgentExecutionStart)
    state_snapshot_events = events(agent_span, AgentSpecStateSnapshotEmitted)
    ...
    assert dump_conversation_state(restored_conversation)["execution"]["status_handled"] is False
...
def test_agent_node_turn_state_snapshots_are_mapped_into_the_agent_span_not_llm_spans() -> None:
    ...
    llm_spans = spans(span_recorder, AgentSpecLlmGenerationSpan)
    assert llm_spans
    assert not any(
