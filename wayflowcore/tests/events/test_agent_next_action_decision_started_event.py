...
from .event_listeners import AgentNextActionDecisionStartEventListener
...
def test_event_creation_with_missing_arguments_fails(missing_attribute: str) -> None:
    ...
    with pytest.raises(ValueError, match=f"An attribute named `{missing_attribute}`"):
        AgentNextActionDecisionStartEvent(**all_attributes)
...
def test_correct_event_serialization_to_tracing_format(
    ...
    event = AgentNextActionDecisionStartEvent(**event_info, execution_state=execution_state)
...
def test_event_is_triggered_with_agent_execute_exit_conversation() -> None:
    ...
    event_listener = AgentNextActionDecisionStartEventListener()
    ...
    assert all(
        isinstance(event, AgentNextActionDecisionStartEvent)
