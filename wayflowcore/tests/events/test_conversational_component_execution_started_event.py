...
from .event_listeners import (
    AgentExecutionStartedEventListener,
    ConversationalComponentExecutionStartedEventListener,
    FlowExecutionStartedEventListener,
)
...
def test_event_creation_with_missing_arguments_fails(
    missing_attribute: str, event_class: Type[ConversationalComponentExecutionStartedEvent]
) -> None:
    all_attributes: Dict[str, Any] = {
        "conversational_component": (
            create_single_step_flow(InputMessageStep(message_template="How are you?"))
            if event_class == FlowExecutionStartedEvent
...
def test_correct_event_serialization_to_tracing_format(
    ...
    if isinstance(event_info["conversational_component"], Agent):
        event = AgentExecutionStartedEvent(**event_info)
    elif isinstance(event_info["conversational_component"], Flow):
        event = FlowExecutionStartedEvent(**event_info)
    else:
        raise ValueError(
...
def test_event_is_triggered_with_flow(flow: Flow) -> None:
    agent_event_listener = AgentExecutionStartedEventListener()
    flow_event_listener = FlowExecutionStartedEventListener()
    ...
    assert isinstance(
        event_listener.triggered_events[0], ConversationalComponentExecutionStartedEvent
    )
...
def test_event_is_triggered_with_agent(
    ...
    assert isinstance(event_listener.triggered_events[0], AgentExecutionStartedEvent)
