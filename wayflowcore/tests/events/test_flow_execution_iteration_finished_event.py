...
from .event_listeners import FlowExecutionIterationFinishedEventListener
...
def test_event_creation_with_missing_arguments_fails(missing_attribute: str) -> None:
    ...
    with pytest.raises(ValueError, match=f"An attribute named `{missing_attribute}`"):
        FlowExecutionIterationFinishedEvent(**all_attributes)
...
def test_correct_event_serialization_to_tracing_format(
    ...
    event = FlowExecutionIterationFinishedEvent(**event_info, execution_state=execution_state)
...
def test_event_is_triggered_with_flow_with_yielding_steps() -> None:
    flow = create_single_step_flow(
        step=InputMessageStep("What's your question?"),
    )

    event_listener = FlowExecutionIterationFinishedEventListener()
    ...
    assert isinstance(event_listener.triggered_events[0], FlowExecutionIterationFinishedEvent)
...
def test_events_are_triggered_with_nested_flows(
    ...
    assert all(
        isinstance(event, FlowExecutionIterationFinishedEvent)
