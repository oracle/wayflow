...
from wayflowcore.events.event import _MASKING_TOKEN, ConversationMessageStreamStartedEvent
from wayflowcore.events.eventlistener import register_event_listeners
from wayflowcore.serialization import serialize_to_dict
from wayflowcore.steps import PromptExecutionStep

from .event_listeners import ConversationMessageStreamStartedEventListener
...
def test_event_creation_with_missing_arguments_fails(missing_attribute: str) -> None:
    ...
    with pytest.raises(ValueError, match=f"An attribute named `{missing_attribute}`"):
        ConversationMessageStreamStartedEvent(**all_attributes)
...
def test_correct_event_serialization_to_tracing_format(
    ...
    event = ConversationMessageStreamStartedEvent(**event_info)
...
def test_event_is_triggered_with_agent(remotely_hosted_llm):
    event_listener = ConversationMessageStreamStartedEventListener()
    ...
    assert isinstance(event_listener.triggered_events[0], ConversationMessageStreamStartedEvent)
