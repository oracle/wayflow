...
async def streaming_tool_impl() -> AsyncGenerator[str, None]:
...
@pytest.mark.anyio
async def test_server_tool_streams_chunks_and_returns_final_result() -> None:
    decorated = tool("streaming_tool", streaming_tool_impl)
...
def test_server_tool_rejects_sync_generators() -> None:
    tool = ServerTool(
        ...
        func=sync_gen_tool_impl,
    )
    step = ToolExecutionStep(tool=tool)

    with pytest.raises(TypeError, match="Synchronous generator tools are not supported"):
...
def test_server_tool_infinite_stream_stops_with_clear_error() -> None:
    ...
    with pytest.raises(
        ...
        _ = asyncio.run(run_step_and_return_outputs_async(step))


def test_server_tool_streams_unlimited_when_max_chunks_is_minus_one() -> None:
    ...
    try:
        ...
        listener = ServerToolStreamingListener()
        with register_event_listeners([listener]):
            outputs = asyncio.run(run_step_and_return_outputs_async(step))
...
class FieldListener(EventListener):
    def __init__(self) -> None:
        self.events: list[ToolExecutionStreamingChunkReceived] = []
...
def test_server_tool_streams_chunks_sync_path_and_event_fields() -> None:
    ...
    field_listener = FieldListener()  # fresh listener
...
def test_agent_with_streaming_tool_and_patched_llm_emits_tool_chunks(big_llama) -> None:
    @tool(description_mode="only_docstring")
    ...
    with patch_llm(llm, outputs=[[ToolRequest("my_stream_tool", {}, "req_123")], "done"]):
        listener = ServerToolStreamingListener()
        with register_event_listeners([listener]):
    ...
    # We should have received the two intermediate chunks
    assert [c for (c, _) in listener.chunks] == ["first", "second"]
    # ^ check earlier streamed chunks were received despite final type error
