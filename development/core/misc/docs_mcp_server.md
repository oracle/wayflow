...
The docs MCP server can also be consumed from WayFlow through [MCPTool](../api/tools.md#mcptool)
and the [StdioTransport](../api/tools.md#stdiotransport).

The example below starts the docs MCP server as a local subprocess, connects a
single `get_docs` tool to an [Agent](../api/agent.md#agent), and lets the agent inspect the
...
#### NOTE
As with the rest of the [MCP guide](../howtoguides/howto_mcp.md),
...
server, the docs recommend [StdioTransport](../api/tools.md#stdiotransport), which is intended
for subprocess-based communication on the same machine. The same guide also points to
the [Tools API](../api/tools.md) for the transport details.
