...
They can be accessed and modified throughout a Flow using [VariableStep](../api/flows.md#variablestep), [VariableReadStep](../api/flows.md#variablereadstep),
and [VariableWriteStep](../api/flows.md#variablewritestep) operations.
[VariableStep](../api/flows.md#variablestep) provides an API to read, write, or update (write and then read) multiple variables in a single step.
[VariableReadStep](../api/flows.md#variablereadstep) and [VariableWriteStep](../api/flows.md#variablewritestep) are simpler steps used exclusively to read from or write to a single variable, respectively.
