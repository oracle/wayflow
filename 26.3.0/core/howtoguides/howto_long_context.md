<a id="top-howtolongmessage"></a>

# How to Enable Agents to Handle Long Contexts![python-icon](_static/icons/python-icon.svg) Download Python Script

Python script/notebook for this guide.

[Long message how-to script](../end_to_end_code_examples/howto_long_context.py)
...
- [Advanced Prompting Techniques](howto_prompttemplate.md)
...
- **Summarizing long messages** using [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform): Individual messages that exceed size limits (such as large tool outputs or long user inputs) are automatically summarized using an LLM.
- **Summarizing long conversations** using [ConversationSummarizationTransform](../api/prompttemplate.md#conversationsummarizationtransform): When conversations become too lengthy, older messages are summarized to maintain context while keeping the total size manageable.
- **Using both transforms together**: Combining both approaches for comprehensive context management.

This guide shows how to use built-in [MessageTransform](../api/prompttemplate.md#messagetransform) objects to reduce the context size in agents.
The same transforms can also be attached directly to [Swarm](../api/agent.md#swarm) and [ManagerWorkers](../api/agent.md#managerworkers)
...
A [MessageTransform](../api/prompttemplate.md#messagetransform) is a transformation applied to a list of messages before they are sent to the LLM powering the agent. You can learn
more about them in the [Advanced Prompting Techniques](howto_prompttemplate.md) guide.

In this guide, we use built-in message transforms to adjust the agent’s chat history by passing them directly to the [Agent](../api/agent.md#agent) constructor.
For [Swarm](../api/agent.md#swarm) and [ManagerWorkers](../api/agent.md#managerworkers), the `transforms` constructor argument behaves the same way:
...
the swarm-level transforms. See [Swarm transform ordering](howto_swarm.md#swarm-transform-ordering) for the full ordering.
...
We use the built-in [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform) to automatically summarize individual messages that exceed a specified size limit, including tool outputs, user messages and images.
...
#### NOTE
When creating [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform) without specifying a `datastore`, it will initialize a default [InMemoryDatastore](../api/datastores.md#inmemorydatastore) for caching, which is only suitable for prototyping. This will raise a user warning indicating that in-memory datastores are not recommended for production systems.
For production systems, you can use [OracleDatabaseDatastore](../api/datastores.md#oracledatabasedatastore) or [PostgresDatabaseDatastore](../api/datastores.md#postgresdatabasedatastore).
...
However, an [InMemoryDatastore](../api/datastores.md#inmemorydatastore) does not preserve cached summary rows across process restarts. If you need summary cache reuse after deserialization in a persistent application, use a database-backed datastore such as [OracleDatabaseDatastore](../api/datastores.md#oracledatabasedatastore) or [PostgresDatabaseDatastore](../api/datastores.md#postgresdatabasedatastore).

Let’s integrate the [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform) into the [Agent](../api/agent.md#agent):
...
In this example, we run a conversation where the agent uses a tool that returns a very long log output. The [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform) automatically summarizes this long message to stay within size limits. The agent then answers questions about the error, demonstrating that key information is preserved despite the summarization.
...
- **After one user message**, both approaches show high token usage:
  - Without [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform): the agent LLM must process the full long tool output (around 12,800 tokens)
  - With [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform): the summarization LLM generates a summary (around 8,900 tokens)
...
This method uses the built-in [ConversationSummarizationTransform](../api/prompttemplate.md#conversationsummarizationtransform) to summarize older messages when conversations become too long, preserving historical information while keeping the context manageable.
...
Let’s integrate the [MessageTransform](../api/prompttemplate.md#messagetransform) into the [Agent](../api/agent.md#agent):
...
If you want to check that the [ConversationSummarizationTransform](../api/prompttemplate.md#conversationsummarizationtransform) is really summarizing old messages together, you either use the same `TokenLister` as in the previous section to measure tokens or you can also create your own `EventListener` that prints the conversation.

## Using Both Transforms Together

For comprehensive context management, you can apply both transforms together. The [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform) will first handle individual long messages, and then the [ConversationSummarizationTransform](../api/prompttemplate.md#conversationsummarizationtransform) will manage the overall conversation length.
...
We have seen in this how-to how to leverage [MessageSummarizationTransform](../api/prompttemplate.md#messagesummarizationtransform) and [ConversationSummarizationTransform](../api/prompttemplate.md#conversationsummarizationtransform) to reduce the size of the LLM context.
We have also seen how to use event listeners to measure LLM token consumption. With this new knowledge, you can proceed to [How to Build Assistants with Tools](howto_build_assistants_with_tools.md).

## Full Code

Click the card at the [top of this page](#top-howtolongmessage) to download the complete code for this guide, or copy it below.
...
# pip install "wayflowcore==26.3.0" 
# ```
...
# %%[markdown]
## Create ConversationSummarizationTransform With Character Threshold
