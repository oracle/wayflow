...
* **Configurable retry policies for remote components**
  ...
  For usage details, see [the new retry configuration guide](howtoguides/howto_retry_configuration.md).
* **Custom TLS certificates for OpenAI-compatible models:**

  [OpenAICompatibleModel](api/llmmodels.md#openaicompatiblemodel) and [OpenAICompatibleEmbeddingModel](api/embeddingmodels.md#openaicompatibleembeddingmodel)
  now support optional `key_file`, `cert_file` and `ca_file` parameters for custom CAs and mutual TLS.

  For more information read [how to configure custom TLS certificates for OpenAI-compatible endpoints](howtoguides/llm_from_different_providers.md#openaicompatible-custom-tls).
* **OAuth support for MCP Clients:**

  MCP Clients now support OAuth-based authorization.

  For more information read the [API Reference](api/auth.md) and the guide on
  [how to connect assistants to MCP servers](howtoguides/howto_mcp.md).
...
* **Gemini models (Vertex AI + AI Studio):**
  ...
  For more information read the [API Reference on LLM models](api/llmmodels.md) and the guide on
...
* **Logprob support in \`LlmGenerationConfig\` and \`PromptExecutionStep\`**
  ...
  For more information please read the guide on [How to request per-token log-probabilities](howtoguides/generation_config.md#request-logprobs)
...
* **Added a new MCP server to browse the WayFlow documentation:**
  ...
  For more information, see the [Docs MCP Server guide](misc/docs_mcp_server.md).
...
* **VariableStep:**
  ...
  For more information check out [how to use variables for shared state in flows](howtoguides/howto_variable.md).
...
* **Fix summarization transform persistence and multi-agent integration**
  ...
  See the guide on [using Swarm](howtoguides/howto_swarm.md#swarm-transform-ordering) and [ManagerWorkers](howtoguides/howto_managerworkers.md#managerworkers-transform-ordering) to learn
...
* **Tool output streaming:**
  ...
  Check the guide on [How to enable tool output streaming](howtoguides/howto_tooloutputstreaming.md#top-tooloutputstreaming) for more information.
...
* **Stricter environment for jinja templates rendering:**
  ...
  Check the guide on [How to write secure prompts with Jinja templating](howtoguides/howto_promptexecutionstep.md#securejinjatemplating) for more information.
...
* **Added Flow Builder to simplify programmatic creation of WayFlow Flows.**
  ...
  For more information, see the [API Reference](api/flows.md) and the [Reference Sheet](misc/reference_sheet.md#flowbuilder-ref-sheet).
...
* **Agent Spec Tracing support:**
  ...
  For more information read the guide on [How to Enable Tracing in WayFlow](howtoguides/howto_tracing.md).
* **WayFlow Plugins:**

  Wayflow plugins allow users extending existing components (like Tools, Steps, etc.), or even creating
  new ones, and seamlessly integrate them in WayFlow and Agent Spec serialization and deserialization. For more information read the guide on [how to use WayFlow plugins](howtoguides/howto_plugins.md).
* **OpenAI Responses API Support:**

  [OpenAICompatibleModel](api/llmmodels.md#openaicompatiblemodel) now supports OpenAI Responses API, which can be configured
  using the `api_type` parameter, which accepts values from [OpenAIAPIType](api/llmmodels.md#openaiapitype).
  ...
  For more information check out [the how-to guide on LLMs from different providers](howtoguides/llm_from_different_providers.md).
* **Serve agents via OpenAI Responses API:**

  Added [OpenAIResponsesServer](api/agentserver.md#openairesponsesserver) and
  [ServerStorageConfig](api/agentserver.md#serverstorageconfig) to host WayFlow agents behind OpenAI
  Responses-compatible endpoints, with optional persistence through supported datastores.

  See [the new how-to guide on serving agents](howtoguides/howto_serve_agents.md) for in-memory
  setup, datastore persistence, and Agent Spec export/reload examples.
* **Serve agents via A2A protocol:**
  Introduced the [A2AServer](api/agentserver.md#a2aserver) class for serving WayFlow conversational components using the A2A protocol.

  For more information check out [how to serve assistants with A2A protocol](howtoguides/howto_a2a_serving.md).
...
* **Support list, dict and tuple output types in MCP tools:**
  ...
  For more information see the guide on [using MCP tools](howtoguides/howto_mcp.md).
* **Python 3.14 support**
...
* **Improve Swarm prompt template, introduce HandoffMode and support multiple tool calls in Swarm:**
  ...
  mechanism when delegating tasks to other agents. Read more at [HandoffMode](api/agent.md#handoffmode).
...
* **Agent Spec structured generation:**
  ...
  For more information check out [the how-to guide on Structured Generation](howtoguides/howto_promptexecutionstep.md)
* **Added Tool Confirmation before Execution:**
  ...
  For more information check out [the corresponding how-to guide](howtoguides/howto_userconfirmation.md)
...
* **Dependency Security Updates:**
  ...
  [GHSA-j975-95f5-7wqh](https://github.com/advisories/GHSA-j975-95f5-7wqh),
  [GHSA-3qhf-m339-9g5v](https://github.com/advisories/GHSA-3qhf-m339-9g5v),
  [GHSA-6757-jp84-gxfx](https://github.com/advisories/GHSA-6757-jp84-gxfx),
  and [GHSA-8q59-q68h-6hv4](https://github.com/advisories/GHSA-8q59-q68h-6hv4).
...
* **Use execution statuses to interact with the components**
  ...
  For more information check out [Reference Sheet](misc/reference_sheet.md)
...
- [How-to Guides](howtoguides/index.md)
